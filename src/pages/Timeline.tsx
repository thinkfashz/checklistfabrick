    ...
    <div className="container mx-auto px-6 py-8">
      <div className="flex justify-between items-center mb-12">
        <div>
          ...
          <h2 className="text-4xl font-headline font-bold tracking-tighter uppercase">CRONOGRAMA</h2>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-sm border border-white/10">
            ...
            <span className="text-xs font-bold uppercase tracking-widest min-w-[100px] text-center">{currentMonth}</span>
        ...
        <div className="lg:col-span-3 space-y-6">
              ...
              {days.map(day => {
                ...
                return (
                  <motion.div 
                    ...
                    className={`aspect-square flex flex-col items-center justify-center rounded-xl border transition-all cursor-pointer relative group ${
                  ...
                  >
                    ...
                    {hasNote && (
                      <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-fabrick-card border border-white/10 p-2 rounded-lg text-[8px] uppercase font-bold text-white opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-20 w-24 text-center">
                        {hasNote}
                      </div>
                    )}
                  </motion.div>
          ...
          {selectedDate && (
            <motion.div 
              ...
              className="glass-panel p-6 border-l-4 border-fabrick-yellow"
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className={`p-2 rounded-lg transition-all ${alarms[selectedDate] ? 'bg-fabrick-yellow text-fabrick-black' : 'bg-white/5 text-fabrick-gray hover:text-white'}`}
                  >
                    <Bell size={18} />
                  </button>
                  <button 
                    onClick={() => setShowNoteModal(true)}
                    className="p-2 bg-white/5 text-fabrick-gray hover:text-white rounded-lg transition-all"
                  >
                    <StickyNote size={18} />
                  </button>
                </div>
              ...
              {deadlines.filter(d => d.date.endsWith(`-${selectedDate < 10 ? '0' + selectedDate : selectedDate}`)).map(d => (
                <div key={d.id} className="flex items-center gap-4 p-4 bg-fabrick-yellow/5 rounded-xl border border-fabrick-yellow/20">
                  <div className="p-2 bg-fabrick-yellow/20 rounded-lg text-fabrick-yellow">
                    <AlertCircle size={20} />
                  </div>
                  <div>
          ...
          <div className="space-y-4">
            {deadlines.map((deadline) => (
              <motion.div 
                ...
                className={`glass-panel p-5 border-l-4 cursor-pointer transition-all ${
      ...
      <AnimatePresence>
        {showNoteModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-headline text-xl font-black uppercase">Añadir Nota</h3>
                <button onClick={() => setShowNoteModal(false)} className="text-fabrick-gray hover:text-white">
                  <X size={20} />
                </button>
              </div>
              <textarea 
                className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm focus:outline-none focus:border-fabrick-yellow transition-all h-32 mb-6"
